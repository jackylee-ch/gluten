...
### Fallbacks
Besides the unsupported operators, functions, file formats and data sources listed in
[Operator and Function Support Progress](./velox-backend-support-progress.md), there are some known
cases that also fall back to vanilla Spark.

#### ANSI
Gluten falls back the whole plan when ANSI mode is enabled. This is controlled by
`spark.gluten.sql.ansiFallback.enabled` (default true); setting it to false lets Gluten attempt to
execute in ANSI mode, which is supported only for basic arithmetic at this point.
...
### Data type restrictions in scan

For the authoritative, per-type view see
[Data type support](./velox-backend-support-progress.md#2-data-type-support).

- ORC scan falls back for `TimestampType`, for `char(n)`-typed strings (when
  `spark.gluten.sql.orc.charType.scan.fallback.enabled` is on, the default), and for nested array with
  struct or array as element type, or nested map with struct as key type or array as value type. See
  `VeloxBackend.validateFormat`.
- Parquet scan has no complex-type restriction: the per-type fallbacks that used to apply were removed
  in [#6712](https://github.com/apache/incubator-gluten/pull/6712). Parquet scan does fall back when
