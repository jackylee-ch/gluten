...
| 标记 | 含义 |
|------|------|
| 支持 | 卸载至 Velox 原生向量化执行 |
| 部分支持 | 可用，但受同行所述条件限制 |
| 不支持 | 该计划节点或表达式回退至原生 Spark |
| — | 不适用 |
...
可向量化（支持 + 部分支持）合计 **316 个，占 81.9%**。

**"支持"列应读作上界。** 至少 4 行标记为支持但会在原生校验阶段被拒（见
[4.3](#43-如何解读函数支持清单)），原因是生成脚本在没有测试触发回退时默认标记为支持。

> **口径说明**：上述逐函数状态取自 Gluten 官方生成的四份函数支持清单（以 Spark 3.5 为基准），
> 再剔除 Spark 3.5 才引入、3.3.2 中不存在的 49 个函数得出。这 49 个函数中 21 个已被 Gluten
> 支持、28 个不支持，因此 3.3.2 的支持比例（81.9%）反而**高于** 3.5（77.5%）——新版本引入的
> 函数尚未全部适配。
...
**反方向做同样的比对**——清单标为支持 / 部分支持、但映射名在原生未注册的函数——得到 3 类假阳性：
...
3. **"支持"是默认值，不是肯定结论。** 除非测试日志中出现匹配的回退原因，生成器一律标记为
   支持（`gen-function-support-docs.py:1174-1185`）。任何未被已启用套件覆盖的函数，都会显示
   为支持——这正是 `split_part`、`approx_percentile`、`percentile_approx`、`percentile` 被误标
   的原因。当某一行对业务确实重要时，应对照 [4.2 节](#42-原生硬性限制)的原生黑名单与白名单
   核实，而非直接采信"支持"。
