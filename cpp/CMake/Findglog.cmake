...
if(NOT BUILD_GLOG)
  ...
  # On macOS, prefer the static libglog.a over libglog.dylib. libvelox.dylib
  # has static gflags baked in via folly, so we must avoid libglog.dylib
  # which transitively loads libgflags.dylib at runtime and triggers the
  # "linked both statically and dynamically" abort in gflags.
...
# On macOS, force static gflags regardless of glog linkage. libvelox.dylib
# has static gflags baked in via folly (-DGFLAGS_SHARED=FALSE). If
# libgluten.dylib were to load libgflags.dylib dynamically, gflags would
# detect the same flag registered both statically (inside libvelox.dylib)
# and dynamically (via libgflags.dylib) and abort with
# "being linked both statically and dynamically".
if(CMAKE_SYSTEM_NAME MATCHES "Darwin")
  set(libgflags_component static)
endif()

# On macOS with a static libglog.a we link glog and gflags via
# -load_hidden so their symbols stay local to libgluten.dylib. This prevents
# dyld from coalescing the weak symbols (C++ function-local statics like
# FlagRegistry::GlobalRegistry) with the static gflags copy already inside
# libvelox.dylib, which otherwise triggers
# "flag 'flagfile' ... linked both statically and dynamically".
#
...
if(_gluten_glog_hidden_load)
  ...
  message(STATUS "Linking gflags (static, -load_hidden): ${_gflags_static_path}")
