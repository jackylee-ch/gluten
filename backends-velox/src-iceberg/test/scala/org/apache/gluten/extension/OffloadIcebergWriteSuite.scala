...
class OffloadIcebergWriteSuite extends AnyFunSuite {
  ...
  private case class AlwaysOffload() extends OffloadIcebergWriteBase {
    override protected def offloadWrite(plan: SparkPlan): SparkPlan = OffloadedPlan()
  }
  ...
  test("write offload rules skip offloading when native write is disabled") {
    withNativeWrite(enabled = false) {
      assert(AlwaysOffload().offload(DummyPlan()) === DummyPlan())
    }
  }

  test("write offload rules offload when native write is enabled") {
    withNativeWrite(enabled = true) {
